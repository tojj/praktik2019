...
export default class FUNDRAISER {
  ...
  async save() {
    let response = await fetch('/json/fundraisers/' + (this._id ? this._id : ''), {
  ...
  async delete() {
    ...
    let response = await fetch('/json/fundraisers/' + this._id, {
      method: "DELETE",
      headers: {
        'Content-Type': 'application/json'
      }
  ...
  static async find(query = '') {        
    let response = await fetch('/json/fundraisers/' + query, {
      method: "GET",
      headers: {
        'Content-Type': 'application/json'
      }
